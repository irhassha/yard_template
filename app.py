...
# =================== SLOT ALLOCATION FUNCTION (Dual Plan) ===================
def allocate_containers_dual_plan(df_vessel):
    allocation_plan_a = []
    allocation_plan_b = []
    yard_blocks = {
        "NP1": ["A01", "A02", "A03", "A04"],
    ...
    for _, row in df_vessel.iterrows():
        ...
        occupied_blocks_today = [alloc["Block"] for alloc in allocation_plan_a if alloc["ETA Vessel"] == eta_vessel]
        ...
        for i in range(cluster_need):
            ...
            shared_slots = [s for s in slots_in_block if s in occupied_slots and 
                            any(alloc["ETA Vessel"] != eta_vessel for alloc in allocation_plan_a if alloc["Block"] == block and alloc["Slot"] == s)]
    ...
    for alloc in allocation_plan_a:
        if any(existing["ETA Vessel"] != alloc["ETA Vessel"] for existing in allocation_plan_a if existing["Block"] == alloc["Block"] and existing["Slot"] == alloc["Slot"]):
            allocation_plan_b.append(alloc)

    return pd.DataFrame(allocation_plan_a), pd.DataFrame(allocation_plan_b)

# =================== STREAMLIT FILE UPLOADER ===================
st.title("Yard Slot Allocation with Plan A & Plan B")
...
if uploaded_file is not None:
    ...
    # Allocate slots based on uploaded data (Plan A & Plan B)
    df_allocation_plan_a, df_allocation_plan_b = allocate_containers_dual_plan(df_vessel_real)
    ...
    visualize_yard(df_allocation_plan_a, "Plan A - Initial Allocation")
    visualize_yard(df_allocation_plan_b, "Plan B - Overlapping Allocation")
